...
def __plot_ivim_maps(D, D_star, f, method_name, plot_flag, save_plot_flag, save_plot_prefix):
    ...
    ax[0].imshow(D_star, cmap="inferno", clim=(0, 0.08)) #TODO: 1.1 ?
    ax[0].set_title('D* pseudo diffusion')
    ax[1].imshow(D, cmap="inferno", clim=(0, 0.002))  # TODO: 0.02? 0.006?
    ax[1].set_title('D diffusion')
    ax[2].imshow(f, cmap="inferno", clim=(0, 1))
    ax[2].set_title('f fraction')
    ...
    for i in range(2):
        for j in range(math.ceil(b_num / 2)):
            if b_rem == 1 and b_id == b_num:
                ax[i, j].axis('off')
                break
            ax[i, j].imshow(dwmri_images[b_id, :, :], cmap='gray', clim=(0, max_value)) #TODO: no clim?
...
def phantom_simulation(b_val, D_val, D_star_val, Fp_val, B0_val, x_wid=64, y_wid=64, rads=None, noise_type='NaN',
    ...
    :param rads:             (Optional, Default: [10,20,30])
                             ...
                             The radii are in ascending order of length, where the value of the first index of the
                             vector is the radius of the inner circle, and Value of the last index of the vector is
                             the radius of the external circle.
...
####################################################################################################################
####################################################################################################################
def ivim_params_estim(dw_mri_images, b, estimator_mode='ALL', image_type_mode='absolute', b_tresh=200,
    ...
    :param dbg_features:     (Optional, Default: 8)
    :return: Images of the IVIM parameters: D, D_star , f
    """
    ...
    #B0_image_min_real_val = np.percentile(B0_image, 5) # TODO: fit better
    #B0_image_min_real_val = 0 # TODO: fit better
    B0_image_min_real_val = 50 # TODO: fit better
    ...
    for j in range(num_of_pixels):
        bsp_prob[j] = ((np.dot(work_images[:, j].T, work_images[:, j]) - np.dot(work_images[:, j].T,
                                                                                bsp_re_image[:, j])) ** 2 / np.dot(
            bsp_re_image[:, j].T, bsp_re_image[:, j])) ** (-b_num / 2)
    if dbg_features is True:
    ...
    for i in range(BSP_max_iterations):
        for j in range(num_of_pixels):
            ...
            if np.dot(tmp_bsp_rnd_rec_pxl.T, tmp_bsp_rnd_rec_pxl) == 0:
                tmp_bsp_rnd_rec_pxl = tmp_bsp_rnd_rec_pxl + 0.0000001
            tmp_bsp_rnd_prob = ((np.dot(work_images[:, j].T, work_images[:, j]) - np.dot(work_images[:, j].T,
                                                                                         tmp_bsp_rnd_rec_pxl)) ** 2 /
                                np.dot(tmp_bsp_rnd_rec_pxl.T, tmp_bsp_rnd_rec_pxl)) ** (-b_num / 2)
...
def error_estim_ivim_maps(dw_mri_images, b_val, known_D, known_D_star, known_f, estim_D, estim_D_star, estim_f,
                          error_type='perc'):
    """
    :param dw_mri_images: 3D matrix - A series of N images. Measured dw-mri images
    :param b_val: 1D vector - N b values that the dw-mri images were taken in.
    :param known_D: obligated matrix. [(mm^2)/s]
    :param known_D_star: obligated matrix. [(mm^2)/s]
    :param known_f: obligated matrix. [a.u.]
    :param estim_D: obligated matrix. [(mm^2)/s]
    :param estim_D_star: obligated matrix. [(mm^2)/s]
    :param estim_f: obligated matrix. [a.u.]
    :param error_type: optional. Defines the error function. could be: l1 or l2 or perc. Default perc.
    :return: D_error, D_star_error, f_error
             mean_error - mean of the above errors
    """
    # ########################################### input errors checker ########################################### #
    input_valid = 'valid'
    if not np.all(np.greater_equal(dw_mri_images, 0)):
    ...
    # =============== roughly rm background =============== #
    known_D_spread     [B0_image_spread < B0_image_min_real_val] = 0
    known_D_star_spread[B0_image_spread < B0_image_min_real_val] = 0
    known_f_spread     [B0_image_spread < B0_image_min_real_val] = 0
    estim_D_spread     [B0_image_spread < B0_image_min_real_val] = 0
    estim_D_star_spread[B0_image_spread < B0_image_min_real_val] = 0
    estim_f_spread     [B0_image_spread < B0_image_min_real_val] = 0
    ...
    if error_type is 'perc':
        ...
        # ~~~ calc perc ~~~ #
        perc_D_error      = np.mean(100 * rel_D)
        perc_D_star_error = np.mean(100 * rel_D_star)
        perc_f_error      = np.mean(100 * rel_f)
...
####################################################################################################################
####################################################################################################################
def error_estim_dw_images(orig_dw_mri_images, rec_dw_mri_images, b_val, error_type='perc'):
    """
    :param orig_dw_mri_images: 3D matrix - A series of N images. Measured dw-mri images
    :param rec_dw_mri_images: 3D matrix - A series of N images. Reconstructed dw-mri images
    :param b_val: 1D vector - N b values that the dw-mri images were taken in.
    :param error_type: optional. Defines the error function. could be: l1 or l2 or perc. Default perc.
    :return: D_error, D_star_error, f_error
             mean_error - mean of the above errors
    ...
    x_wid = B0_image.shape[2]
    y_wid = B0_image.shape[1]
    b_val = np.array(b_val)
    b_num = b_val.size()
    B0_image_min_real_val = np.percentile(B0_image, 5)  # TODO: fit better
    # prep images to analysis
    ...
    for i in range(b_num):
        ...
        tmp_rec = np.reshape(orig_dw_mri_images[i,:,:], (x_wid * y_wid))
        ...
        if error_type is 'perc':
            ...
            # ~~~ calc perc ~~~ #
            perc_error_list = perc_error_list+[np.mean(100 * rel)]
